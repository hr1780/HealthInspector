...
<br><br><br><br>
         ...
         // System.out.println(m_doc+m_city+m_sep);
          if(m_sep==null){
        	  pst = con.prepareStatement("select * from regdoctor where name like ?");
          	  pst.setString(1,"%"+m_doc+"%");
          }
          else{
        	  pst = con.prepareStatement("select * from regdoctor where name like ? and address like ? and Specialization like ?");
          	  pst.setString(1,"%"+m_doc+"%");
...
<div>
 <pre>
<h2><font size="12"><u><%=rs.getString(2) %></u></font><br>
Specialization  : <%= rs.getString(3) %>   
Shift : <%=rs.getString(4) %>                 
Phone No. : <%= rs.getLong(5) %>
City   :<%= rs.getString(6) %>               
  </h2> </pre>
   <center>
         <a href="booking.jsp?value=<%=rs.getString(2)%>&value2=<%= rs.getString(6) %>" class="button">BOOK APPOINTMENT</a></center>
 </div>
 </td>
 </tr>         
<tr></tr><tr></tr><tr></tr>
